...
function incidentsReport(data) {  
  ...
  for (var i=0; i<incidentlength; i++) {
    ...
    latlong[i] = {"incident_id": incidentdata.id, "city":incidents_city, "area":incidents_area, "latitude":incidentdata.latitude, "longitude":incidentdata.longitude, "categories":incidents_categories, "dateTime":ChangeDateFormat(incidents_date,1)+" | "+timeConvert(incidents_time)};
...
function addMarkersToMap(markers) {
  ...
  for (let marker of markers) {
    ...
    let mapMarker = new google.maps.Marker({
      ...
      html: "<span class='map-grey'>Type : </span><span class='map-grey map-dark'>#"+marker.incident_id+' '+marker.categories+'</span><br>'+"<span class='map-grey'>Location : </span><span class='map-grey map-dark'>"+marker.area+', '+marker.city+'</span><br>'+"<span class='map-grey'>Date & Time : </span><span class='map-grey map-dark'>"+marker.dateTime+'</span>',
      animation: google.maps.Animation.DROP,
      //animation: 'DROP',
      //draggable: true,
