...
//Incidents API
var topIncidentsData = '';
function load_reportedincident(reported_incident_data) {
  //console.log("load reported_incident_data: ", reported_incident_data);
  if(topIncidentsData!='' && reported_incident_data['map_zoom']<=15) {
      setIncidentListings(reported_incident_data, topIncidentsData);
      return true;
  }
  $.ajax({
    type: "POST",
    ...
    success:function(data) {
      if(reported_incident_data['map_zoom']<=15)
        topIncidentsData = data;
      setIncidentListings(reported_incident_data, data);
    }
  });
}

function setIncidentListings(reported_incident_data, data) {
...
function incidentsReport(data) {
  ...
  for (var i=0; i<incidentlength; i++) {
    ...
    var incidents_time = '';
    if(incidentdata.time_to === null) {
      incidents_time = timeConvert(incidentdata.time_from);
    }
    else {
      incidents_time = timeConvert(incidentdata.time_from)+" - "+timeConvert(incidentdata.time_to);
    }
    ...
    for (var j=0; j<incidentdata_answers_length; j++) {
      ...
      var incident_answer_tag = incidentdata_answers[j].answer_id != null ? incidentdata_answers[j].answer_tag : "";
      if (incident_question_tag == "attack_reason") {
        ...
        if(incidentdata_answers[j].other_answers.length != 2){
            var incident_other_answer = JSON.parse(incidentdata_answers[j].other_answers)[incident_question_answer_id] != null ? JSON.parse(incidentdata_answers[j].other_answers)[incident_question_answer_id] : "";
        }
        var policeHtml = '<img src="assets/images/map-police.svg" class="img-fluid"> ';
        var reported_to_police = '';
        if(incident_answer_tag == "reported_police_yes"){
            reported_to_police = policeHtml+' '+home_police_filed;
        } else if(incident_answer_tag == "reported_police_no"){
            reported_to_police = policeHtml+' '+home_police_not_filed;
        } else if(incident_answer_tag == "reported_police_i_tried"){
            reported_to_police = policeHtml+' '+home_police_tried_filed+'. '+incident_other_answer;
        } else if(incident_answer_tag == "reported_police_i_will"){
            reported_to_police = policeHtml+' '+home_police_intend_filed;
        } else if(incident_answer_tag == "reported_police_not_sure"){
        ...
        <div class="longDesc" id="longDesc_${i}" data-id="${i}">
          ...
          <div class="otherDetails">`;
            ...
            <div class="row mb-3">
              <div class="col-md-12">
                <div class="iTime">
                  <img src="assets/images/time-of-incident.svg" class="img-fluid"> ${incidents_time_est} ${incidents_time}
                </div>
              </div>
            </div>
          </div>
...
/*function loadMap(incidentdata) {
  removeMarkers();
  var datalen = incidentdata.length;
  // Set Marker Data
  latlongs = [];
  console.log(incidentdata);
  for (var i=0; i<datalen; i++) {
    latlongs[i] = {"incident_id": incidentdata[i].id, "city":'', "area":'', "lat":parseFloat(incidentdata[i].latitude), "lng":parseFloat(incidentdata[i].longitude), "categories":'', "dateTime":'', "incident_viewmore_id":i};
  }
  addClusterIncidentMarkersToMap(latlongs);
}*/

function load_incident_coordinates(reported_incident_data) {
  //removeMarkers();
  $.ajax({
    type: "POST",
    url: baseURL+'api/reported-incidents/map-coordinates',
    data: reported_incident_data,
    success:function(data) {
      removeMarkers();
      //console.log("load map pointer success: ", data);
      if ((data.status == true)) {
        if (data.data) {
          var datalen = data.data.length;
        }
        if (datalen > 0) {
          ...
          //console.log(incidentdata);
          for (var i=0; i<datalen; i++) {
            latlongs[i] = {"incident_id": incidentdata[i].id, "lat":parseFloat(incidentdata[i].latitude), "lng":parseFloat(incidentdata[i].longitude)};
          }
          addClusterIncidentMarkersToMap(latlongs);
        }
      }
      load_reportedincident(reported_incident_data);
    }
  });
}
...
function incident_viewmore_init(incident_id, infoWin) {
  //console.log("init incident_id", incident_id);
  $.ajax({
    type: "POST",
    ...
    success:function(data) {
      if(data.data.id == incident_id) {
        ...
        if(map_data) {
          ...
          infoWin.setContent(map_htmlContent);
          incident_viewmore(data);
        }
      }
    }
...
function addClusterIncidentMarkersToMap(locations) {
  ...
  var markers = locations.map(function(location, i) {
    var marker = new google.maps.Marker({
      position: location,
      //html: locations[i].incident_id,
      //id: locations[i].incident_id,
      //animation: google.maps.Animation.DROP,
    ...
    marker.set("id", locations[i].incident_id);
    
    google.maps.event.addListener(marker, 'click', function(evt) {
      incident_viewmore_init(this.id, infoWin); //To show details of incident
...
function removeMarkers() {
    for(j=0; j<markerClustererList.length; j++) {
        ...
        markerClustererList[j].clearMarkers();
    }
...
function incident_viewmore(data) {
  ...
  var incidentdata = data.data != null ? data.data : "";
  //console.log("incidentdata: ", incidentlength, incidentdata);
  var incident_desc = incidentdata.description != null ? incidentdata.description : "";
  var additional_info = incidentdata.additional_detail != null ? '<p class="reason">'+incident_additional_details+' : '+incidentdata.additional_detail+'</p>' : "";
  ...
  //var incidents_time = incidentdata.time_to != null ? timeConvert(incidentdata.time_from)+" - "+timeConvert(incidentdata.time_to) : timeConvert(incidentdata.time_from);
  var incidents_area = incidentdata.area != null ? incidentdata.area : "";
  var incidents_city = incidentdata.city != null ? incidentdata.city : "";
  var incidents_state = incidentdata.state != null ? incidentdata.state : "";
  var incidents_categories = incidentdata.categories != null ? incidentdata.categories : "";

  var incidentdata_answers = incidentdata.answers != null ? incidentdata.answers : "";
  var incidentdata_answers_length = incidentdata_answers.length;
  ...
  popup_elementHtml += `
        <div id="incident-viewmore-${incidentdata.id}" class="modal fade incident-popup" role="dialog" data-keyboard="false" data-backdrop="static">
              ...
              <div class="row mb-3">
                <div class="col-md-12">
                  <div class="iTime">
                    ...
                    ${incidents_time_est} ${incidents_time}
                  </div>
                </div>
              </div>
            </div>
