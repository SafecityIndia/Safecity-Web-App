  ...
  $.ajax({
    ...
    success:function(data) {
      ...
      if ("categories_ids" in reported_incident_data) {
        ...
        $("#incidents_list").html('<div class="no-incident-found">No incidents found on these days.</div>');
      }
      else if ("reported_time" in reported_incident_data) {
        $("#incidents_list").html('<div class="no-incident-found">No incidents found in this timeframe.</div>');
      }
      else {
        $("#incidents_list").html('<div class="no-incident-found">No incidents found for this area.</div>');
      }
...
function incidentsReport(data) {
  ...
  for (var i=0; i<incidentlength; i++) {
    ...
    var additional_info = incidentdata.additional_detail != null ? '<p class="reason">Additional Details : '+incidentdata.additional_detail+'</p>' : "";
    ...
    for (var j=0; j<incidentdata_answers_length; j++) {
      ...
      if (incident_question_tag == "attack_reason") {
        var attack_reason = '<p class="reason">What led to the attack : '+incident_question_answer+'</p>';
      }else if (incident_question_tag == "who_was_perpetrator") {
        var perpetrator_info = '<p class="reason">Who was the perpetrator : '+incident_question_answer+'</p>';
      }
        ...
        var medical_help_html = '<div class="row mb-3">'+
                                  '<div class="col-md-12">'+
                                    '<div class="location">'+
                                        medical_help
                                    '</div>'+
