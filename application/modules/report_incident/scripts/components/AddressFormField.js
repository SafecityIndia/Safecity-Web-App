var addressFormField = (function() {
	...
	var latitude  = $.cookie('lat') || 19.076090;
	...
	function nextClick() {
		...
		if(latitude=='' || longitude=='' || area=='' || country=='' || isAccepted==false) {
	...
	function showAddress() {
		...
		if(latitude!='' && longitude!='' && area!='' && country!='' && isAccepted) {
	...
	return function(data, properties, lastQuestion) {
		...
		var elementHtml = `
		 	<div class="form-group mb-4">
		 	  <label class="themeColor m-0 p-0 d-block">${address_locate_on_map}<span class="error">*</span></label>
		 	   <label class="mb-2 p-0 sub-label d-block">${address_locate_on_map_subtext}</label>
		 	  <input type="text" class="form-control form-text" id="search_address" name="" placeholder="${map_start_typing}" value="">
			</div>

			<div class="form-group mb-4">
		 	  <label class="themeColor m-0 p-0 d-block">${address_pin_exact_location}</label>
		 	   <label class="mb-3 p-0 sub-label d-block">${address_pin_exact_location_subtext}</label>
				<div class="mapouter" style="height:467px">
				</div>
			</div>

			<div class="form-group mb-4">
		 	  <label class="themeColor m-0 p-0 d-block">${address_pinned_on_map}:</label>
		 	   <p class="m-0 pt-0 pinned-add"></p>
			</div>

			<div class="form-group mb-4">
		 	  <label class="themeColor mb-2 p-0 d-block">${address_area}<span class="error">*</span></label>
		 	  <input type="text" class="form-control form-text" id="area" placeholder="${address_area_subtext}" value="">
			</div>

			<div class="form-group mb-4">
		 	  <label class="themeColor mb-2 p-0 d-block">${address_build_loc}</label>
		 	  <input type="text" class="form-control form-text" id="building_address" placeholder="${address_build_loc_subtext}" value="">
			</div>

			<p class="mt-4 pt-2 lignheight20">
				${address_i_confirm_msg}
			</p>
			...
			<div class="custom-control custom-checkbox estimate mt-1">
				...
				<label class="custom-control-label eLabel" for="confirm_address">${address_i_confirm}</label>
			</div>
		 `;
