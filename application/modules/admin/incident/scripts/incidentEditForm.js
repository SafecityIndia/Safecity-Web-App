var incidentEditForm = (function () {
	...
	function init(incidentData) {
		...
		$.ajax({
			url: baseURL + "api/get-questions-options",
			type: "POST",
	...
	$(document).on("click", ".dynamic-radio-suboption[type=radio]", function (event) {
		var suboptionid = $(this).attr('id');
		// Handle other textbox toggle
		if($(this).data('hassubtext')==true) {
	...
	function renderQuestion(questionsObj, answerObj, incidentData) {
		...
		if(questionsObj[question_id]==undefined)
			return '';
			...
			case "radio":
				options.forEach(function (option) {
					...
					if (option.textbox_placeholder != null)
						...
						optionHtml += renderSubOptions(option.id,answerObj,
							question_id,
							answerIdArr,
							hassuboption,
							elemsuboptions,
	...
	function renderSubOptions(optionID,answerObj,
		question_id,
		answerIdArr,
		hassuboption,
		elemsuboptions,
		...
		elemsuboptions.forEach(function (elemsuboption) {
			...
			if (hassuboption == "checkbox") {
			  ...
			  if(elemsuboption.textbox_placeholder==null){
					optionHtml += `
		                <div class="custom-control custom-radio">
		                  	<input type="radio" id="suboption${
								elemsuboption.id
							}" name="suboption${question_id}" class="custom-control-input suboption-field dynamic-radio-suboption"  value="${elemsuboption.id}" data-hassubtext="${
						 elemsuboption.textbox_placeholder != null
					     }" ${otherAnswer[answerIdArr] == undefined ? "checked" : ""}>
		            ...
		            `;
		            optionHtml += `
				        <input type="text" name="option${elemsuboption.id}textbox" class="form-control ${
							otherAnswer[answerIdArr] ? "" : "d-none"
						} dynamic-radio-textbox-suboption" value="${
						otherAnswer[answerIdArr] != undefined && optionID==answerIdArr ? otherAnswer[answerIdArr] : ""
						}" placeholder="${elemsuboption.textbox_placeholder}" />
			         `;
			 }
		   }
		});
		...
		$(".question-listing-item").each(function (index, el) {
			...
			switch (type) {
				...
				case "radio":
					...
					if($("input[name="+"suboption"+ questionId +"]:checked").data('hassubtext')==true) {
						var radionId=$("input[name="+"suboption"+ questionId +"]:checked").val();
						var radionIdMain=$("input[name="+"option"+ questionId +"]:checked").val();
						radio_other_answers[radionIdMain] = $("input[name=option"+radionId+"textbox").val();
					}
		...
		$(".question-listing-item").each(function (index, el) {
			...
			if (type == "text" || type == "incident-address-form" || type=="estimate-datepicker") {
				...
				$textElement.each(function(subIndex, subEl) {
					...
					if(type == "incident-address-form") {
						$err_msg_elem = $(subEl).siblings(".invalid-msg");
						if($(subEl).hasClass('is-invalid')) {
							$err_msg_elem.text("This Field is Required");
						} else {
							$err_msg_elem.text("");
						}
					}
				...
				if(type != "incident-address-form") {
					$err_msg_elem = $(el).children(".invalid-msg");
					if(textInvalid) {
						$err_msg_elem.text("This Field is Required");
					} else {
						$err_msg_elem.text("");
