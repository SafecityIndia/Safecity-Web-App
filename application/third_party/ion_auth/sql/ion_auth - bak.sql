...
CREATE TABLE `admin_role` (
  ...
  KEY `fk_admin_role_roles1_idx` (`group_id`),
  CONSTRAINT `uc_admin_role` UNIQUE (`admin_id`, `group_id`),
  CONSTRAINT `fk_admin_role_admins1` FOREIGN KEY (`admin_id`) REFERENCES `admins` (`id`) ON DELETE CASCADE ON UPDATE NO ACTION,
  CONSTRAINT `fk_admin_role_roles1` FOREIGN KEY (`group_id`) REFERENCES `roles` (`id`) ON DELETE CASCADE ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

INSERT INTO `admin_role` (`id`, `admin_id`, `group_id`) VALUES
     (1,1,1),
     (2,1,2);
